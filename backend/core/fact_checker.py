...
class FactChecker:
    ...
    async def extract_claims_async(self, query: str, x_link=None, facebook_link=None, instagram_link=None,
    ...
    def _extract_claims(self, query: str, x_link=None, facebook_link=None, instagram_link=None,
                        youtube_link=None, image_file=None, video_file=None, generic_link=None):
        """
        ...
        print("\n" + "="*80)
        print(f"CLAIM EXTRACTION PROCESS STARTED AT: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("="*80)
    ...
    async def fact_check_claims_async(self, claims: List[str]):
        ...
        with concurrent.futures.ThreadPoolExecutor() as pool:
            ...
            result = await loop.run_in_executor(
                pool, 
                lambda: self._fact_check_claims(claims)
            )
